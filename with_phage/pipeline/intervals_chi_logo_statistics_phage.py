...
# 1. Creates BED files with equal intervals of length 1000 and 10000 nt for the cell genome and 1000 nt for phage genome.
...
phage = pd.read_csv('../reference/phage.fa', sep='\t')
phage.columns = ['fasta']
phage = str(phage['fasta'].sum()).upper()
phage_length = len(phage)
...
def create_bed(interval, dna, dna_name):
    ...
    if dna == phage:
    	file_name = '../phage_coverage/intervals.bed'
    else:
...
# Save phage GC-content in a separate TXT file.
with open('../logo/total_phage_GC.txt', 'w') as ouf:
    ouf.write(str(GC_content(phage)))
    ouf.write('\n')
...
plus_genome = reads.query('flag == 0 and chr != "plasmid" and chr != "phage"')
...
plus_phage = reads.query('flag == 0 and chr == "phage"')
plus_phage['start_position'] = plus_phage['position']

minus_genome = reads.query('flag == 16 and chr != "plasmid" and chr != "phage"')
...
minus_phage = reads.query('flag == 16 and chr == "phage"')
minus_phage['start_position'] = minus_phage['position'] + minus_phage['length'] - 1
...
for start_position in plus_phage['start_position']:
    if start_position >= 16 and start_position <= (phage_length - 30):
        plus_phage_logo.append(phage[start_position - 16 : start_position + 29])
...
for start_position in minus_phage['start_position']:
    if start_position >= 30 and start_position <= (phage_length - 16):
        minus_phage_logo.append(reverse_complement(phage[start_position - 30 : start_position + 15]))
...
phage_logo = plus_phage_logo + minus_phage_logo
phage_GC = []

for nucleotide in range(len(phage_logo[0])):
    position = str()
    for read in phage_logo:
        position += read[nucleotide]
    phage_GC.append(GC_content(position))
...
GC = pd.DataFrame({'position': positions, 'genome': genome_GC, 'plasmid': plasmid_GC, 'phage': phage_GC})
...
for coordinate in range(genome_length - 8):
...
for coordinate in range(8, genome_length):
...
# Calculate all positions of chi-sequences in the positive strand of phage genome and save them to TXT file.
plus_strand_chi_phage = []

for coordinate in range(phage_length - 8):
	if phage[coordinate:(coordinate + 8)] == chi_sequence:
		plus_strand_chi_phage.append(coordinate)

plus_df = pd.DataFrame(plus_strand_chi_phage)
plus_df.to_csv('../phage_coverage/plus_chi.txt', header=False, index=False, sep='\t')

# Calculate all positions of chi-sequences in the negative strand of phage genome and save them to TXT file.
minus_strand_chi_phage = []

for coordinate in range(8, phage_length):
	if phage[coordinate:(coordinate + 8)] == reverse_complement(chi_sequence):
		minus_strand_chi_phage.append(coordinate)

minus_df = pd.DataFrame(minus_strand_chi_phage)
minus_df.to_csv('../phage_coverage/minus_chi.txt', header=False, index=False, sep='\t')
...
plasmid_copy_number = 12 # insert plasmid copy number!
...
with open('../alignment/aligned_on_phage.txt', 'r', encoding='utf-8') as inf:
	for line in inf:
		aligned_on_phage = int(line.strip())
...
with open('../alignment/statistics.tsv', 'w') as ouf:
    ...
    ouf.write('\t')
    ouf.write(str(aligned_on_plasmid + aligned_on_genome + aligned_on_phage))
    ouf.write('\n')
    ...
    ouf.write('\n')
    ouf.write('phage dna length')
    ouf.write('\t')
    ouf.write(str(phage_length))
    ouf.write('\n')
    ouf.write('reads mapped to phage genome')
    ouf.write('\t')
    ouf.write(str(aligned_on_phage))
    ouf.write('\n')
    ouf.write('phage:genome ratio')
    ouf.write('\t')
    ouf.write(str(aligned_on_phage / aligned_on_genome))
    ouf.write('\n')
