...
total_phage_GC <- read.table("total_phage_GC.txt", header = F)
total_phage_GC <- total_phage_GC[,1]
...
ggplot(table, aes(x = position)) +
  geom_vline(xintercept = 1, linetype = "dashed", colour = "grey30", size = 1) +
  geom_hline(yintercept = total_phage_GC, linetype = "dotted", colour = "grey30", size = 1) +
  geom_point(aes(y = phage), col = 'grey30', size = 4) +
  geom_line(aes(y = phage), col = 'grey30', size = 2) +
  scale_x_continuous(breaks = c(-15, -10, -5, 1, 5, 10, 15, 20, 25, 30)) +
  expand_limits(y = c(30, 70)) +
  scale_y_continuous(breaks = c(30, 40, 50, 60, 70)) +
  ylab("GC, %") +
...
ggsave("GC_phage.png", width = 10, height = 5, dpi = 300)
